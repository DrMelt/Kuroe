...
| `Mode` | `Single` 整叶一个 agent，`PerItem` 按规划条目各派一个，只能写在叶子上 |
| `From` | 上下文取自哪些更早叶子的产出 |
...
启动时逐条校验，全部非法项一次给出。规则即任务推进依赖的不变量：节点名不能为空且流程内唯一；`From` 只能引用作用域内更早的叶子，容器节点不可被引用，也不能声明 `From` 与 `Prompt`；按条目展开的叶子必须排在规划叶子之后并连成一段，段后只能有一个收拢检查叶子收尾；每条按条目展开的实施必须被某个检查叶引用；流程必须有检查叶，检查叶必须用 `From` 引用被它检查的实施产出；收拢检查（展开后的整体检查）不支持待批准门控。
