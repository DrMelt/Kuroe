...
static class PlanItems
{
    ...
    public static ErrorOr<IReadOnlyList<PlanItem>> Parse(string itemsJson)
    {
        ...
        {
            return [WorkflowErrors.Items($"不是合法 JSON：{ex.Message}")];
        }

        if (parsed is null || parsed.Count == 0)
        {
            return [WorkflowErrors.Items("条目为空")];
        }

        if (parsed.Count > Limit)
        {
            return [WorkflowErrors.Items($"条目 {parsed.Count} 个，超过上限 {Limit}")];
        }
        ...
        for (int index = 0; index < parsed.Count; index++)
        {
            ...
            {
                return [WorkflowErrors.Items($"第 {index + 1} 个条目缺 Title 或 Instruction")];
            }
