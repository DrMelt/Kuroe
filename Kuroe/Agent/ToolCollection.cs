...
public sealed class ToolCollection(IEnumerable<IAgentTool> providers)
{
    ...
    [
        .. providers.SelectMany(provider => provider.GetType()
            ...
            .Select(method => (AITool)AIFunctionFactory.Create(method, method.IsStatic ? null : provider))),
    ];
}
