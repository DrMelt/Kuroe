...
internal sealed class KuroeHarness : IDisposable
{
    ...
    private KuroeHarness(string root, ServiceProvider provider, FakeExecutor executor)
    {
        ...
        Driver = provider.GetRequiredService<WorkflowDriver>();
    ...
    public WorkflowDriver Driver { get; }
    ...
    public static ErrorOr<KuroeHarness> TryCreate(string? flowsJson = null)
    {
    ...
    public static KuroeHarness Create(string? flowsJson = null) => TryCreate(flowsJson).ThrowIfError();

    public TaskId Submit(string goal, string? flow = null) =>
        Driver.Submit(goal, flow, null).ThrowIfError().Id;
    ...
    public void Dispose()
    {
        Driver.ShutdownAsync().GetAwaiter().GetResult();
        _provider.Dispose();
